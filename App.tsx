  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-8 text-center">
        ...
        <h2 className="text-3xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-emerald-400 to-indigo-400">Configurando seu Painel PRO...</h2>
        <p className="text-slate-500 max-w-sm">Preparando seu motor de ganhos e treinos de elite.</p>
      </div>
    );
  }
              ...
              <div className="space-y-6 animate-fade-in">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm">Painel de Embaixador Ativo.</p>
                  </div>
                  <button 
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div onClick={() => setActiveTab('referral')} className="bg-indigo-500/10 border border-indigo-500/20 p-6 rounded-3xl cursor-pointer hover:bg-indigo-500/20 transition-all">
                    ...
                    <h3 className="font-bold text-white">Ver Minha Rede</h3>
                    <p className="text-xs text-slate-500 mt-1">{userProfile.network.length} Parceiros ativos</p>
                  </div>
                </div>

                <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6">
                   <h4 className="font-bold text-sm mb-4 uppercase tracking-widest text-slate-500">Resumo de Performance</h4>
                   <div className="grid grid-cols-2 gap-y-4 text-sm">
                      <div className="flex flex-col"><span className="text-slate-500 text-xs">Código</span> <span className="font-mono text-emerald-400 font-bold">{userProfile.referralCode}</span></div>
                      <div className="flex flex-col"><span className="text-slate-500 text-xs">Membros</span> <span className="font-medium text-white">{userProfile.referralCount} ativos</span></div>
                   </div>
                </div>

                {/* Botão Atalho para Plano de Marketing */}
                <button 
                ...
                >
                  ...
                  <span className="font-bold text-slate-300">Conheça o Plano de Carreira 3 Níveis</span>
        ...
        <nav className="fixed bottom-6 inset-x-4 max-w-md mx-auto z-40">
          <div className="bg-slate-900/80 backdrop-blur-2xl border border-slate-800 rounded-full p-2 flex justify-between shadow-2xl">
            ...
            <button onClick={() => setActiveTab('workout')} className={`flex-1 flex flex-col items-center py-2 rounded-full transition-all ${activeTab === 'workout' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/40' : 'text-slate-500'}`}>
              ...
              <span className="text-[10px] font-bold mt-1">Treinos</span>
            </button>
            <button onClick={() => setActiveTab('referral')} className={`flex-1 flex flex-col items-center py-2 rounded-full transition-all ${['referral', 'marketing_plan'].includes(activeTab) ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-slate-500'}`}>
              ...
              <span className="text-[10px] font-bold mt-1">Ganhos</span>
            </button>
